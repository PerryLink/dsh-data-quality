...
- **Release status (2026-08-19)**: `v0.1.0` released — local `node scripts/release.mjs 0.1.0` (gate re-run + commit + annotated tag), pushed to `PerryLink/dsh-data-quality` (`main`); the `Release` workflow passed its gate (112 tests) and created the GitHub Release from the CHANGELOG section. npm publish was skipped (no `NPM_TOKEN` secret); add the secret to the repo and re-run the workflow (`POST /actions/runs/<id>/rerun`) to publish, since `dsh-data-quality@0.1.0` is not on the registry yet.
