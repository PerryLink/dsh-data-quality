...
| 组件 | 版本 |
|---|---|
| DeepSeek Harness | `dsh-v0.1.7-alpha.1`（2026-09-18 已适配）：peer 区间已接纳 alpha.2 线；该线上 `Session.append` 的第三参仅对表面事件类型存在且为 `SurfaceIntent`，因此审计门控仍为跳过、storage-domain 报告仍是持久副本。已于 2026-09-18 核验（双 typecheck 尺子 + 全量测试绿）。 |
