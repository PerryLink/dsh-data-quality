...
- `test/` — vitest; REAL `Context`/`SessionStore`/`Session`/`SystemPrompt`/`ToolRuntime` and the REAL storage seam (dsh-storage + dsh-storage-json + dsh-storage-domain) from the 0.1.7-alpha.2 peers. No hand-written service mocks. Engine specs are pure.
