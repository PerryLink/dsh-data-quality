...
| Component | Version |
|---|---|
| DeepSeek Harness | `0.1.2-rc.1` (adapted 2026-09-02): the session envelope keeps its ignorable field for stored-log read compatibility only - Session.append still cannot stamp it, so audit-gate behavior is unchanged. Verified 2026-09-06 against the dsh-v0.1.3-alpha.1 master checkout (full gate chain + profile install smoke). |
...
- Tests run vitest against the REAL `Context`/`Session`/`ToolRuntime`/storage domain from the 0.1.2-rc.1 peers (no hand-written service mocks) plus pure engine specs; every clean/verify rule has positive and negative cases, and `verifyCitations` covers all four statuses.
